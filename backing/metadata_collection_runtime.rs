...
fn handle_persistence_target(
    ...
    match op {
        ...
        "store_szz_analysis_result" => store_szz_analysis_result(
            &input,
            &arg_value(args, 1, context),
        ),
...
fn compute_score_bucket(input: &Value) -> Result<Value, ApiError> {
        ...
        .unwrap_or_default()
    {
        ...
        if let Some(details) = result.get_mut("details").and_then(Value::as_object_mut) {
            ...
            details.insert("effective_weight".to_string(), json!(round_decimal(effective_weight, 4)));
            details.insert("weighted_value".to_string(), json!(round_decimal(weighted_value, 4)));
        }
        results.push(result);
    }
...
fn store_szz_analysis_result(input: &Value, provider_run: &Value) -> Result<Value, ApiError> {
    if provider_run.is_null() {
        return Err(ApiError::provider("SZZ provider returned an empty response"));
    }
    ...
    for name in [
        ...
        if let Ok(raw) = env::var(name) {
            ...
            return serde_json::from_str(raw).map_err(|err| {
                ApiError::internal(format!("{name} contains invalid JSON: {err}"))
            });
        }
    }
    for name in [
...
mod tests {
    ...
    fn resolves_saves_and_adjusts_keyword_configs() {
        let _guard = isolated();
        let resolved = resolve_keyword_config(&json!({ "repository_id": "openstack/neutron" })).unwrap();
